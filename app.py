...
def add_translation(input_text, translated_text, source_lang, target_lang):
    conn = sqlite3.connect("users_and_history.db")
    c = conn.cursor()
    c.execute("INSERT INTO history (input_text, translated_text, source_lang, target_lang) VALUES (?, ?, ?, ?)", 
              (input_text, translated_text, source_lang, target_lang))
    conn.commit()
    conn.close()
...
def process_audio_with_translation(page, audio_file_path, output_text):
...
def translate_and_speak_text(tts_textbox, trans_langbox, tts_translated_text, page):
...
def main(page: ft.Page):
    ...
    def speech_to_text_view():
        ...
        def stop_listening(e):
            ...
            if is_listening:
                ...
                if os.path.exists(audio_file_path):
                    transcript = process_audio_with_translation(page, audio_file_path, output_text)
        ...
        def translate_speech(e):
            translate_and_speak_text(stt_textbox, trans_langbox, stt_translation, page)
            page.update()
            ...
            content=ft.Column(
                [
                    ft.Text("Text-To-Speech Converter", size=25, weight="bold", color=ft.colors.RED),
                            ...
                            trans_langbox,
                            ft.ElevatedButton(text="Translate and Speak", on_click=lambda e: translate_and_speak_text(tts_textbox, trans_langbox, tts_translated_text, page)),
                            tts_translated_text,
    ...
    def history_view():
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ft.Text("Translation History", size=24, weight="bold"),
                    *[ft.Text(f"{item[1]} -> {item[2]} ({item[3]} to {item[4]})") for item in history_list]
                ],
                expand=True
            ),
            padding=20
